...
class refPage extends StatefulWidget {
  final int subjectId;
  const refPage({super.key, required this.subjectId});
...
class _refPageState extends State<refPage> {
  ...
  void initState() {
    super.initState();
    sitesLoader(widget.subjectId).then((data) {
      setState(() {
        isRefreshing = false;
        sites = data;

        // Group sites by title
  ...
  void refresh() async {
    ...
    try {
      await refreshData(context,widget.subjectId.toString(),"sites");
      await sitesLoader(widget.subjectId).then((data)=>{
        setState(() {
          sites = data;
          groupedSites = _groupSitesByTitle(sites);
          isRefreshing = false;
